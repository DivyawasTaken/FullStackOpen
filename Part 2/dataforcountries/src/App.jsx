...
const Weather = ({ name, latitude, longitude }) => {
  ...
  useEffect(() => {
    ...
    .then(response => {
      setTemp(response.main.temp -273.13)
      setWeather(response.weather[0].description)
      setSpeedWind(response.wind.speed)
      seticon_url('https://openweathermap.org/img/wn/' + response.weather[0].icon + '@4x.png')
    })
  },[])
  ...
  return (
    <div>
      ...
      <img src={icon_url} width={350} height={350} />
    </div>
  )
}

const C_info = ({dataInfo, show}) =>{
  // edge case
  if (show === false) {
    return null
  }
...
const Country = ({country}) => {
  ...
  return (
    <div>
      ...
      <C_info dataInfo={country} show={show}/>
    </div>
  )
}
const CountriesList = ({countriesArray}) => {
  if (countriesArray.length >10) {
    ...
    return (
      ...
      return (
        <div>
          <C_info dataInfo={countriesArray[0]} show={true}/>
        </div>
      )
    }
}
